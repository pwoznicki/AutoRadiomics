...
def describe_stat(labels, preds):
    ...
    sens_all, spec_all = [], []
    for i in range(1000):
        boot_preds, boot_labels = resample(preds, labels, replace=True, n_samples=len(labels), random_state=i)
        sens, spec = get_stat(boot_labels, boot_preds)
        sens_all.append(sens)
        spec_all.append(spec)
    sens_all, spec_all = np.array(sens_all), np.array(spec_all)

    print(np.mean(sens_all), np.percentile(sens_all, 2.5), np.percentile(sens_all, 97.5))
    print(np.mean(spec_all), np.percentile(spec_all, 2.5), np.percentile(spec_all, 97.5))
