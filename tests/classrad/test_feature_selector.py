...
class TestFeatureSelector:
    ...
    def test_anova_selection(self, df):
        ...
        assert isinstance(feature_selector.selected_features, list)
        assert len(feature_selector.selected_features) == 5
        assert type(feature_selector.selected_features[0]) == str
        assert set(feature_selector.selected_features).issubset(set(X.columns))
    ...
    def test_lasso_selection(self, df):
        ...
        assert isinstance(feature_selector.selected_features, list)
        assert len(feature_selector.selected_features) >= 0
        assert set(feature_selector.selected_features).issubset(set(X.columns))
